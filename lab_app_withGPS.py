...
def live_tracking():
    try:
        ...
        ser.close()
        return render_template("error.html", error=str(e))  # Assuming you have an error.html template
    return render_template("live_tracking.html", lat=gps_data['lat'], long=gps_data['long'])
...
def send_at(command,back,timeout):
    ...
    if rec_buff != '':
        if back not in rec_buff.decode():
            return render_template ("gps_not_ready.html")
        else:
            global GPSDATA
            ...
            gps_data['lat'] = FinalLat
            gps_data['long'] = FinalLong
            return render_template("live_tracking.html", lat=gps_data['lat'], long=gps_data['long'])
    else:
        return render_template("gps_not_ready.html")
